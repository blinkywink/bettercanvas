...
const FALLBACK_DATABASE_URL =
  'postgresql://neondb_owner:npg_0EC5dtJHuAvi@ep-jolly-morning-ams2l5ga.c-5.us-east-1.aws.neon.tech/neondb?sslmode=require';
...
type AuthenticatedRequest = Request & { userId?: number; username?: string };
...
async function getSession(req: Request): Promise<{ userId: number; username: string } | null> {
  await ensureSchema();
...
async function requireUser(req: AuthenticatedRequest): Promise<{ userId: number; username: string } | null> {
...
function normalizeProfileId(req: Request): string {
  const p = req.query.profile;
  return typeof p === 'string' && p.trim() ? p.trim() : DEFAULT_PROFILE_ID;
}

function parseEventsFromIcsText(icsText: string) {
  ...
  for (const key in events) {
    ...
    if (typeof ev.url === 'string') url = ev.url;
    else if (ev.url && typeof ev.url === 'object' && 'val' in ev.url) url = String((ev.url as any).val);
    out.push({
      uid: ev.uid || key,
...
async function ensureDefaultProfile(userId: number): Promise<void> {
  const p = getPool();
  await p.query(
    `INSERT INTO profiles(user_id, id, name)
     VALUES ($1, $2, $3)
     ON CONFLICT (user_id, id) DO NOTHING`,
    [userId, DEFAULT_PROFILE_ID, 'Default'],
  );
}

export function registerWebAuthRoutes(app: Express): void {
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      if (r.rowCount !== 1) return res.status(401).json({ error: 'Invalid username or password' });
      const row = r.rows[0];
...
export async function webAuthApiGuard(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  (req as AuthenticatedRequest).userId = s.userId;
  (req as AuthenticatedRequest).username = s.username;
  next();
}

export function sendWebLoginPage(res: Response): void {
...
export async function isWebAuthenticated(req: Request): Promise<boolean> {
  return Boolean(await getSession(req));
}

export function registerWebDataRoutes(app: Express): void {
  app.get('/api/profiles', async (req: Request, res: Response) => {
    const s = await requireUser(req as AuthenticatedRequest);
    if (!s) return res.status(401).json({ error: 'Unauthorized' });
    await ensureDefaultProfile(s.userId);
    const r = await getPool().query(
      `SELECT id, name, created_at AS "createdAt" FROM profiles WHERE user_id = $1 ORDER BY created_at ASC`,
      [s.userId],
    );
  ...
  app.put('/api/profiles/:profileId', async (req: Request, res: Response) => {
    ...
    if (!id || !name) return res.status(400).json({ error: 'Invalid profile/name' });
  ...
  app.post('/api/upload', upload.single('icsFile'), async (req: Request, res: Response) => {
    const s = await requireUser(req as AuthenticatedRequest);
    if (!s) return res.status(401).json({ error: 'Unauthorized' });
    ...
    if (!file?.buffer) return res.status(400).json({ error: 'icsFile required' });
    ...
    if (!isUpdate) {
      await p.query(
        `INSERT INTO profiles(user_id, id, name) VALUES ($1, $2, $3) ON CONFLICT (user_id, id) DO UPDATE SET name = EXCLUDED.name`,
        [s.userId, profileId, name],
      );
    }
    await p.query(
      `INSERT INTO profile_ics(user_id, profile_id, ics_text) VALUES ($1, $2, $3)
       ON CONFLICT (user_id, profile_id) DO UPDATE SET ics_text = EXCLUDED.ics_text, updated_at = NOW()`,
      [s.userId, profileId, icsText],
    );
  ...
  app.get('/api/events', async (req: Request, res: Response) => {
    ...
    if (r.rowCount !== 1) return res.json([]);
    try {
  ...
  app.get('/api/custom-events', async (req: Request, res: Response) => {
    const s = await requireUser(req as AuthenticatedRequest);
    if (!s) return res.status(401).json({ error: 'Unauthorized' });
    const profileId = normalizeProfileId(req);
    const r = await getPool().query(
      `SELECT event_json FROM custom_events WHERE user_id = $1 AND profile_id = $2 ORDER BY uid ASC`,
      [s.userId, profileId],
    );
    return res.json(r.rows.map((x: { event_json: unknown }) => x.event_json));
  });

  app.post('/api/custom-events', async (req: Request, res: Response) => {
    const s = await requireUser(req as AuthenticatedRequest);
    if (!s) return res.status(401).json({ error: 'Unauthorized' });
    const profileId = normalizeProfileId(req);
    ...
    await getPool().query(
      `INSERT INTO custom_events(user_id, profile_id, uid, event_json) VALUES ($1, $2, $3, $4::jsonb)`,
      [s.userId, profileId, uid, JSON.stringify(event)],
    );
    return res.json({ ok: true, event });
  });

  app.put('/api/custom-events/:uid', async (req: Request, res: Response) => {
    const s = await requireUser(req as AuthenticatedRequest);
    if (!s) return res.status(401).json({ error: 'Unauthorized' });
    const profileId = normalizeProfileId(req);
    const uid = String(req.params.uid || '');
    const existing = await getPool().query(
      `SELECT event_json FROM custom_events WHERE user_id = $1 AND profile_id = $2 AND uid = $3`,
      [s.userId, profileId, uid],
    );
    if (existing.rowCount !== 1) return res.status(404).json({ error: 'Not found' });
    const merged = { ...(existing.rows[0].event_json || {}), ...(req.body || {}), uid };
    await getPool().query(
      `UPDATE custom_events SET event_json = $4::jsonb WHERE user_id = $1 AND profile_id = $2 AND uid = $3`,
      [s.userId, profileId, uid, JSON.stringify(merged)],
    );
    return res.json({ ok: true });
  });

  app.delete('/api/custom-events/:uid', async (req: Request, res: Response) => {
    const s = await requireUser(req as AuthenticatedRequest);
    if (!s) return res.status(401).json({ error: 'Unauthorized' });
    const profileId = normalizeProfileId(req);
    const uid = String(req.params.uid || '');
  ...
  for (const key of stateKeys) {
    app.get(`/api/user-state/${key}`, async (req: Request, res: Response) => {
      const s = await requireUser(req as AuthenticatedRequest);
      if (!s) return res.status(401).json({ error: 'Unauthorized' });
      const profileId = normalizeProfileId(req);
      const r = await getPool().query(
        `SELECT value FROM user_state WHERE user_id = $1 AND profile_id = $2 AND key = $3`,
        [s.userId, profileId, key],
      );
      if (r.rowCount !== 1) return res.json(key === 'initialized' ? { initialized: false } : {});
      return res.json(r.rows[0].value);
    });

    app.put(`/api/user-state/${key}`, async (req: Request, res: Response) => {
      const s = await requireUser(req as AuthenticatedRequest);
      if (!s) return res.status(401).json({ error: 'Unauthorized' });
      const profileId = normalizeProfileId(req);
      await getPool().query(
        `INSERT INTO user_state(user_id, profile_id, key, value) VALUES ($1, $2, $3, $4::jsonb)
         ON CONFLICT (user_id, profile_id, key) DO UPDATE SET value = EXCLUDED.value`,
        [s.userId, profileId, key, JSON.stringify(req.body ?? {})],
      );
      return res.json({ ok: true });
    });
  }

  app.get('/api/user-state/profile', async (req: Request, res: Response) => {
    const s = await requireUser(req as AuthenticatedRequest);
    if (!s) return res.status(401).json({ error: 'Unauthorized' });
  ...
  app.put('/api/user-state/profile', async (req: Request, res: Response) => {
    const s = await requireUser(req as AuthenticatedRequest);
    if (!s) return res.status(401).json({ error: 'Unauthorized' });
